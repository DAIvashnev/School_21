...
int s21_sub_equal(s21_decimal value_1, s21_decimal value_2,
  ...
  while (!is_zero(value_2)) {
    *result = s21_and_dec_inverted(value_1, value_2);
    ...
    if (is_super_zero(svalue_1) && !is_super_zero(svalue_2)) {
      ...
      *result = value_1;
      ...
      if (getsign(value_1) && getsign(value_2)) {
        ...
        s21_add_super(svalue_2, svalue_1, &sresult);
        ...
        if (s21_is_super_less_or_equal(svalue_1, svalue_2)) {
          err = s21_sub_super_equal(svalue_2, svalue_1, &sresult);
          negative = 1;

        } else {
          err = s21_sub_super_equal(svalue_1, svalue_2, &sresult);
        }
      }
...
int s21_sub_super_equal(s21_super_decimal value_1, s21_super_decimal value_2,
  ...
  while (!is_super_zero(value_2)) {
    *result = s21_super_and_dec_inverted(value_1, value_2);
