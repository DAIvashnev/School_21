...
void s21_grep_help(const char *argv, t_st *structData) {
  ...
  if ((argv[0] == '-' && structData->help_grep <= 2) ||
      ((structData->e == 1 || structData->f == 1) &&
       structData->help_grep <= 2)) {
...
void checking_key(t_st *structData) {
  for (size_t i = 1; structData->key[i] != 0 && structData->error_options == 0;
       i++) {
...
void regexData(regex_t *re, int *error_regex, t_st *structData) {
  if ((structData->e == 1 || structData->f == 1) && structData->i == 1 &&
      (*error_regex = regcomp(re, structData->pattern,
                              REG_NEWLINE | REG_EXTENDED | REG_ICASE)) == 0) {
  } else if ((structData->e == 1 || structData->f == 1) && structData->i == 0 &&
             (*error_regex = regcomp(re, structData->pattern,
                                     REG_NEWLINE | REG_EXTENDED)) == 0) {
  } else if ((structData->e == 0 && structData->f == 0) && structData->i == 1 &&
             (*error_regex = regcomp(re, structData->pattern,
...
int checking_file(t_st *structData, int *countFile, int *check_s) {
  ...
  while ((structData->file[*countFile] != '\0') && flag == 0) {
    ...
    j++;
    *countFile += 1;
    if (structData->file[*countFile] == ' ' ||
        structData->file[*countFile] == '\0') {
      *countFile += 1;
  ...
  while (fgets(structData->check_str, 256, fp) != NULL) {
    if ((*error_regex = regexec(re, structData->check_str, 0, match, 0)) == 0 &&
      ...
      structData->countOutput += 1;
                    ...
                    regexec(re, structData->check_str, 0, match, 0)) != 0 &&
               structData->v == 1) {
      structData->countOutput += 1;
    }
  }
  ...
  while (fgets(structData->check_str, 256, fp) != NULL) {
    ...
    if ((*error_regex = regexec(re, structData->check_str, 0, match, 0)) == 0) {
      for (int i = 0; structData->o_pattern[i] != '\0'; i++) {
        structData->o_parsing[j] = structData->o_pattern[i];
        j++;
        if (structData->o_pattern[i + 1] == '|' ||
            structData->o_pattern[i + 1] == '\0') {
          structData->o_parsing[j] = '\0';
          int count = strlen(structData->o_parsing);
          if (structData->o_parsing[count - 1] == '$' ||
              structData->o_parsing[0] == '^' ||
              structData->o_parsing[0] == '\\') {
            o_parsing(structData, &count);
          }
          for (int k = 0; structData->check_str[k] != '\0'; k++) {
            if (structData->i == 1) {
              if (toupper(structData->o_parsing[x]) ==
                      structData->check_str[k] ||
                  tolower(structData->o_parsing[x]) ==
                      structData->check_str[k]) {
              ...
              if (structData->o_parsing[x] == structData->check_str[k]) {
                structData->o_search[x] = structData->check_str[k];
                x++;
              } else {
                x = 0;
            ...
            if (structData->o_parsing[x] == '\0') {
              structData->o_search[x] = '\0';
              x = 0;
              if (strcasecmp(structData->o_search, structData->o_parsing) ==
                  0) {
                if (structData->n == 1) {
                  count_output(structData);
                } else {
...
void o_parsing(t_st *structData, const int *count) {
  ...
  int j = 1;
  if (structData->o_parsing[*count - 1] == '$' &&
      structData->o_parsing[*count - 2] != '\\') {
    structData->o_parsing[*count - 1] = '\0';
  } else if (structData->o_parsing[*count - 1] == '$' &&
             structData->o_parsing[*count - 2] == '\\') {
    c = structData->o_parsing[*count - 1];
    structData->o_parsing[*count - 2] = c;
    structData->o_parsing[*count - 1] = '\0';
  }
  if (structData->o_parsing[0] == '^' && structData->o_parsing[1] != '\\') {
    while (j != *count) {
      structData->o_parsing[i] = structData->o_parsing[j];
      i++;
      j++;
    }
    structData->o_parsing[i] = '\0';
  } else if (structData->o_parsing[0] == '\\') {
    while (i != *count) {
      c = structData->o_parsing[j];
      structData->o_parsing[i] = c;
      i++;
      j++;
    }
    structData->o_parsing[i] = '\0';
...
void output(t_st *structData) {
  if (structData->countArgument > 1 && structData->h == 0) {
    if (structData->o == 1 && structData->v == 0 && structData->c == 0 &&
        structData->l == 0) {
...
void count_output(t_st *structData) {
  if (structData->countArgument > 1 && structData->h == 0) {
    if (structData->o == 1 && structData->v == 0 && structData->c == 0 &&
      ...
      printf("%s:%d:%s", structData->check_file, structData->countOutput,
             structData->check_str);
    }
  } else {
    if (structData->o == 1 && structData->v == 0 && structData->c == 0 &&
        structData->l == 0) {
